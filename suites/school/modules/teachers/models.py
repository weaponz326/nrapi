...
def teacher_upload_path(instance, filename):
    ...
    return 'school/{}/modules/teacher/{}'.format(instance.account.id, filename)
...
class Teacher(CustomBaseModel):
    ...
    photo = models.FileField(null=True, upload_to=teacher_upload_path)
    ...
    teacher_code = models.CharField(max_length=32, null=True, blank=True)
    staff_grade = models.CharField(max_length=256, null=True, blank=True)

    class Meta:
        db_table = 'school_module_teacher'

    def __str__(self):
        return str(self.id)

class TeacherCodeConfig(CustomBaseModel):
    ...
    class Meta:
        db_table = 'school_module_teacher_code_config'
