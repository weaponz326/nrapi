...
from .serializers import LedgerCodeConfigSerializer, LedgerSerializer, LedgerItemSerializer
...
class LedgerCodeConfigDetailView(APIView):
    permission_classes = (IsAuthenticated,)

    def get(self, request, id, format=None):
        code = LedgerCodeConfig.objects.get(id=id)
        serializer = LedgerCodeConfigSerializer(code)
        return Response(serializer.data)

    def put(self, request, id, format=None):
        code = LedgerCodeConfig.objects.get(id=id)
        serializer = LedgerCodeConfigSerializer(code, data=request.data, partial=True)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors)

class NewLedgerCodeConfigView(APIView):
    permission_classes = (IsAuthenticated,)

    def get(self, request, id, format=None):
        code_set = LedgerCodeConfig.objects.get(id=id)
        new_code = generate_code(code_set.last_code)                

        if code_set.entry_mode == 'Auto':
            code = LedgerCodeConfig.objects.filter(id=id)
...
def save_extended_profile(sender, instance, created, **kwargs):
    if created:
        LedgerCodeConfig.objects.create(
            id=instance.id,
            entry_mode="Auto",
