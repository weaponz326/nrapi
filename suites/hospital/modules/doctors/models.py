...
def doctor_upload_path(instance, filename):
    ...
    return 'hospital/{}/modules/doctor/{}'.format(instance.account.id, filename)
...
class Doctor(CustomBaseModel):
    ...
    photo = models.FileField(null=True, upload_to=doctor_upload_path)
    ...
    doctor_code = models.CharField(max_length=32, null=True, blank=True)
    ...
    class Meta:
        db_table = 'hospital_module_doctor'

    def __str__(self):
        return str(self.id)

class DoctorCodeConfig(CustomBaseModel):
